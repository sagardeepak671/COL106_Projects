            ...
            for end in cities:
                if start != end:
                    dynamic_cases.extend([
                        ...
                        TestCase(
                            f"Dynamic_{start}_{end}_Peak",
                            f"Peak time route from {start} to {end}",
                            start, end,
                            min_time + (max_time - min_time)//4,
                            min_time + 3*(max_time - min_time)//4
                        )
                    ])
...
# Generated using flight data patterns
    ...
    def validate_route(route, test_case, route_name, all_valid_routes):
        ...
        if route_name == "Route 1 (Least Flights, Earliest)":
            ...
            if sum(f.fare for f in route) > min_fare:
